...
class Preprocesser(object):
    ...
    def readVQA(
        self, datasetFilename, instancesFilename, tier, updateVocab, imageIndex=None
    ):
    ...
    def encodeQuestionStr(self, questionStr):
        return self.vqaProcessText(questionStr, True, True)
    ...
    def readData(
        self, datasetFilename, instancesFilename, tier, updateVocab, imageIndex
    ):
        # data extraction
        datasetReader = {
            "VG": self.readVG,
        ...
        return datasetReader[config.dataset](
            datasetFilename, instancesFilename, tier, updateVocab, imageIndex
        )
    ...
    def readTier(self, tier, train):
        ...
        if tier == "val" and config.valFilenames != []:
            datasetFilename = [config.datasetFile(tier)] + [
                config.dataFile(vf) for vf in config.valFilenames
            ]
        ...
        instances = self.readData(
            datasetFilename, instancesFilename, tier, train, imageIndex
        )  # updateVocab =
    ...
    def readDataset(self, suffix="", hasTrain=True, trainOnVal=False):
        ...
        if trainOnVal:
            ...
            if config.valSplit > 0:
                dataset["train"]["instances"] += dataset["val"]["instances"][
                    config.valSplit :
                ]
                dataset["val"]["instances"] = dataset["val"]["instances"][
                    : config.valSplit
                ]
        ...
        if (
            config.dataset == "VQA" and config.ansFormat == "oe"
        ):  # and (not config.ansAddUnk):
    ...
    def vectorizeData(self, data):
        ...
        if config.dataset == "VQA":
            ...
            answerFreqs, answerFreqsN = vectorize2DList(
                encodedAnswerFreqs, minY=config.AnswerFreqMaxNum
            )

            vectorizedData.update(
                {"answerFreqs": answerFreqs, "answerFreqNums": answerFreqsN}
            )

            if config.ansFormat == "mc":
                encodedChoices = [aDict.encodeSeq(d["choices"]) for d in data]
                choices, choicesN = vectorize2DList(
                    encodedChoices, minY=config.choicesMaxNum
                )

                vectorizedData.update({"choices": choices, "choicesNums": choicesN})
    ...
    def prepareData(self, data, train, filterKey=None, noBucket=False):
        ...
        filterTrain = {
            "maxQLength": config.tMaxQ,
            "maxPLength": config.tMaxP,
            "onlyChain": config.tOnlyChain,
            "filterOp": config.tFilterOp,
        }

        filterVal = {
            "maxQLength": config.vMaxQ,
            "maxPLength": config.vMaxP,
            "onlyChain": config.vOnlyChain,
            "filterOp": config.vFilterOp,
        }

        filters = {
            "train": filterTrain,
            "evalTrain": filterTrain,
            "val": filterVal,
            "test": filterDefault,
        }
        ...
        if fltr["onlyChain"]:
            data = [
                d
                for d in data
                if all((len(inputNum) < 2) for inputNum in d["programInputs"])
            ]
    ...
    def prepareDataset(self, dataset, noBucket=False):
        ...
        for tier in dataset:
            if dataset[tier] is not None:
                dataset[tier]["data"] = self.prepareData(
                    dataset[tier]["instances"],
                    train=dataset[tier]["train"],
                    filterKey=tier,
                    noBucket=noBucket,
                )
    ...
    def initEmbRandom(self, num, dim):
        ...
        if config.wrdEmbUniform:
            ...
            embeddings = np.random.uniform(low=lowInit, high=highInit, size=(num, dim))
    ...
    def initializeWordEmbeddings(self, dim, wordsDict=None, random=False, name=""):
            ...
            if not random:
                ...
                with open(config.wordVectorsFile, "r") as inFile:
                    ...
                    for sym, idx in wordsDict.sym2id.items():
                        if " " in sym:
                            symEmb = self.sentenceEmb(
                                sym, wordVectors, config.wrdAEmbDim
                            )
        ...
        if (
            wordsDict.padding in wordsDict.sym2id and wordsDict != self.answerDict
        ):  # ?? padding for answers?
    ...
    (when config.ansEmbMod == "BOTH"). If config.ansEmbMod == "SHARED", tie
    embeddings for question and answer same symbols.
    """

    def initializeQAEmbeddings(self):
        # use same embeddings for questions and answers
        if config.ansEmbMod == "SHARED":
            qaEmbeddings = self.initializeWordEmbeddings(
                config.wrdQEmbDim, self.qaDict, random=config.wrdEmbQRandom, name="qa"
            )
            oeAnswers = np.array(
                [self.qaDict.sym2id[sym] for sym in self.answerDict.id2sym]
            )  # [config.answerDelta:]
            embeddings = {"qa": qaEmbeddings, "oeAnswers": oeAnswers}
        # use different embeddings for questions and answers
        else:
            qEmbeddings = self.initializeWordEmbeddings(
                config.wrdQEmbDim,
                self.questionDict,
                random=config.wrdEmbQRandom,
                name="q",
            )
            aEmbeddings = None
            if config.ansEmbMod == "BOTH":
                aEmbeddings = self.initializeWordEmbeddings(
                    config.wrdAEmbDim,
                    self.answerDict,
                    random=config.wrdEmbARandom,
                    name="a",
                )
    ...
    Preprocesses a given dataset into numpy arrays:
    1. Reads the input data files into dictionary.
    2. Saves the results jsons in files and loads them instead of parsing input
    if files exist
    3. Initializes word embeddings to random / GloVe.
    4. Optionally filters data according to given filters.
    5. Encodes and vectorize the data into numpy arrays.
    ...
    def preprocessData(self, debug=False):
        ...
        extraDataset = self.prepareDataset(
            extraDataset, noBucket=(not config.extraVal) or (not config.alterExtra)
        )
