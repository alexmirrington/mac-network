...
class MACnet(object):
    ...
    def addPlaceholders(self):
        with tf.variable_scope("Placeholders"):
            ...
            if config.ansFormat == "mc":
                ...
                # in general could consolidate that with mc and make it more
                # general if i did choicesIndices all of them in case of open ended
    ...
    def createFeedDict(self, data, images, train):
        ...
        if config.imageObjects:
            ...
            feedDict.update({self.imagesPlaceholder: images["images"]})
        ...
        with tf.variable_scope("stem"):
            ...
            if config.imageObjects:  # VQA ??? or config.useBaseline:
                ...
                if config.stemLinear:
                    features = ops.linear(
                        images, inDim, outDim, dropout=self.dropouts["stem"]
                    )
                    dim = outDim
                elif config.stemDeep:
                    dims = [inDim] + config.stemDims + [outDim]
                    features = ops.FCLayer(
                        features, dims, dropout=self.dropouts["stem"]
                    )
            ...
            if config.stemLinear:
                ...
                if config.stemNumLayers == 0:
                    ...
                    dims = (
                        [inDim]
                        + ([config.stemDim] * (config.stemNumLayers - 1))
                        + [outDim]
                    )
    ...
    def qEmbeddingsOp(self, qIndices, embInit):
        with tf.variable_scope("qEmbeddings"):
            ...
            embeddingsVar = tf.get_variable(
                ...
                initializer=embInit,
                dtype=tf.float32,
                trainable=(not config.wrdEmbQFixed),
            )
            embeddings = tf.concat(
                [tf.zeros((1, config.wrdQEmbDim)), embeddingsVar], axis=0
            )
    ...
    def aEmbeddingsOp(self, aIndices, embInit):
        with tf.variable_scope("aEmbeddings"):
            ...
            embeddings = tf.get_variable(
                ...
                initializer=embInit,
                dtype=tf.float32,
                trainable=(not config.wrdEmbAFixed),
            )
    ...
    def vocabEmbeddings(self, embInit, name):
        with tf.variable_scope("vocabEmbeddings" + name):
            ...
            embeddings = tf.get_variable(
                ...
                initializer=embInit,
                dtype=tf.float32,
                trainable=(not config.semanticFixEmbs),
            )
        return embeddings
    ...
    def embeddingsOp(self, qIndices, aIndices, embInit):
        ...
        if config.ansEmbMod == "SHARED":
            ...
            questions, answers, qaEmbeddings = self.qaEmbeddingsOp(
                qIndices, aIndices, embInit["qa"]
            )
        else:
    ...
    def encoder(
        ...
        with tf.variable_scope("encoder"):
            ...
            if config.encVariationalDropout:
                varDp = {
                    "stateDp": self.dropouts["stateInput"],
            ...
            for i in range(config.encNumLayers):
                questionCntxWords, vecQuestions = ops.RNNLayer(
                    ...
                    dropout=self.dropouts["encInput"],
                    varDp=varDp,
                    name="rnn%d" % i,
                )
            ...
            if projWords:
                questionCntxWords = ops.linear(
                    questionCntxWords, config.encDim, projDim, name="projCW"
                )
            if projQuestion:
                vecQuestions = ops.linear(
                    vecQuestions,
                    config.encDim,
                    projDim,
                    act=config.encProjQAct,
                    name="projQ",
                )
    ...
    def baselineAttLayer(self, images, memory, inDim, hDim, name="", reuse=None):
        with tf.variable_scope("attLayer" + name, reuse=reuse):
            ...
            # projMemory = tf.expand_dims(
            #    ops.linear(memory, inDim, hDim, name = "projMemory"), axis = -2
            # )
            # if config.saMultiplicative:
            #     interactions = projImages * projMemory
            # else:
            #     interactions = tf.tanh(projImages + projMemory)
            interactions, hDim = ops.mul(
                images,
                memory,
                inDim,
                proj={"dim": hDim, "shared": False},
        ...
        with tf.variable_scope("baseline"):
            if config.baselineAtt:
                ...
                for i in range(config.baselineAttNumLayers):
                    memory = self.baselineAttLayer(
                        images, memory, hDim, hDim, name="baseline%d" % i
                    )
                memDim = hDim
            else:
                if config.imageObjects:
                    ...
                    attention = ops.inter2att(
                        interactions, hDim, mask=self.imagesObjectNum
                    )
                    images = ops.att2Smry(attention, images)
                else:
                    images, imageDim = ops.linearizeFeatures(
                        images, self.H, self.W, imageDim, projDim=config.baselineProjDim
                    )
    ...
    By default the classifier input will be the the final memory state of the
    MAC network.
        ...
        with tf.variable_scope("outputUnit"):
            ...
            if config.outQuestion:
                ...
                features, dim = ops.concat(
                    features, eQ, config.memDim, mul=config.outQuestionMul
                )
            ...
            if config.outImage:
                ...
                images = ops.linear(
                    images, config.memDim, config.outImageDim, name="outImage"
                )
    ...
    Optionally uses answer word embeddings in computing the logits
    ...
    Args:
        ...
        aEmbedding: supported word embeddings for answer words in case answerMod
        is not NON. Optionally computes logits by computing dot-product with
        answer embeddings.
        ...
        with tf.variable_scope("classifier"):
            ...
            if config.answerMod != "NON":
                ...
                interactions, interDim = ops.mul(
                    choices, logits, dims[-1], interMod=config.answerMod, concat=concat
                )
                logits = ops.inter2logits(
                    interactions, interDim, sumMod=config.answerSumMod
                )
                if config.ansFormat == "oe":
    ...
    def addAnswerLossOp(self, logits, answers, answerFreqs, answerFreqNums):
        if config.lossType == "softmax":  # or config.ansFormat == "mc":
            ...
            answerDist = answerFreqs / tf.expand_dims(
                tf.to_float(answerFreqNums), axis=-1
            )
            ...
            if config.lossWeight == 1:
                ...
                losses = tf.nn.weighted_cross_entropy_with_logits(
                    targets=answerDist, logits=logits, pos_weight=config.lossWeight
                )
    ...
    # Computes predictions (by finding maximal logit value, corresponding to
    # highest probability) and mean accuracy between predictions and answers.
    def addPredOp(self, logits, answers):  # , answerFreqs
        with tf.variable_scope("pred"):
            if config.ansFormat == "oe":  # and config.ansAddUnk:
                mask = tf.to_float(tf.sequence_mask([2], config.answerWordsNum)) * (
                    -1e30
                )  # 1 or 2?
                logits += mask
            ...
            if config.dataset == "VQA" and config.ansFormat == "oe":
                agreeing = tf.reduce_sum(
                    tf.one_hot(preds, config.answerWordsNum) * self.answerFreqs, axis=-1
                )
    ...
    def addOptimizerOp(self):
        with tf.variable_scope("trainAddOptimizer"):
            ...
            if config.subsetOpt:
                self.subsetOptimizer = tf.train.AdamOptimizer(
                    learning_rate=self.lr * config.subsetOptMult
                )
    ...
    def computeGradients(
        self, optimizer, loss, trainableVars=None
        ...
        with tf.variable_scope("computeGradients"):
            ...
            if config.subsetOpt:
                self.subset_gradients_vars = self.subsetOptimizer.compute_gradients(
                    loss, subsetVars
                )
        ...
        with tf.variable_scope("train"):
            ...
            if config.clipGradients:
                clippedGradients, _ = tf.clip_by_global_norm(
                    gradients, config.gradMaxNorm, use_norm=norm
                )
            ...
            with tf.control_dependencies(updateOps):
                ...
                if config.subsetOpt:
                    subsetTrain = self.subsetOptimizer.apply_gradients(
    ...
    def averageAcrossTowers(self, gpusNum):
        if gpusNum == 1:
            ...
            self.answerLossAll = tf.reduce_mean(
                tf.stack(self.answerLossList, axis=0), axis=0
            )
            self.answerAccAll = tf.reduce_mean(
                tf.stack(self.answerAccList, axis=0), axis=0
            )
            self.correctNumAll = tf.reduce_sum(
                tf.stack(self.correctNumList, axis=0), axis=0
            )
            ...
            if config.subsetOpt:
                ...
                for grads_var in zip(*self.subset_gradientVarsList):
                    gradients, variables = zip(*grads_var)
                    if gradients[0] is not None:
                        avgGradient = tf.reduce_mean(
                            tf.stack(gradients, axis=0), axis=0
                        )
                    else:
                        avgGradient = None
                    var = variables[0]
    ...
    def trimData(self, data):
        data["questions"] = self.trim2DVectors(
            data["questions"], data["questionLengths"]
        )
        return data
    ...
    def buildPredsList(self, data, predictions, attentionMaps):
        ...
        for i, instance in enumerate(data["instances"]):
            ...
            def attMapToList(attMap):
                return [step[i].tolist() for step in attMap]
    ...
    def runBatch(
        self, sess, data, images, train, getPreds=False, getAtt=False, allData=None
    ):
        ...
        attOp = (
            self.macCell.attentions
            if not config.useBaseline
            else (self.attentions if config.baselineNew else self.noOp)
        )
        ...
        if getPreds:
            ...
            predsList = self.buildPredsList(
                data, predsInfo[0], attentionMaps if getAtt else None
            )
    ...
    def build(self):
            ...
            for i in range(config.gpusNum):
                with tf.device("/gpu:{}".format(i)):
                    with tf.name_scope("tower{}".format(i)):
                        ...
                        questionWords, choices = self.embeddingsOp(
                            self.questionIndices,
                            self.choicesIndices,
                            self.embeddingsInit,
                        )

                        projWords = projQuestion = (
                            config.encDim != config.ctrlDim
                        ) or config.encProj
                        questionCntxWords, vecQuestions = self.encoder(
                            questionWords,
                            self.questionLengths,
                            projWords,
                            projQuestion,
                            config.ctrlDim,
                        )
                        ...
                        imageFeatures, imageDim = self.stem(
                            self.images, self.imageInDim, config.memDim
                        )

                        # baseline model
                        if config.useBaseline:
                            # inpImg = imageFeatures if config.baselineNew \
                            #     else self.images
                            # inpDim = imageDim if config.baselineNew \
                            #     else self.imageInDim
                        ...
                        # MAC model
                            ...
                            finalControl, finalMemory = self.MACnetwork(
                                imageFeatures,
                                vecQuestions,
                                questionWords,
                                questionCntxWords,
                        ...
                        if config.dataset == "VQA":
                            self.answerFreqs = self.aggregateFreqs(
                                self.answerFreqLists, self.answerFreqNums
                            )
                        else:
                        ...
                        self.preds, self.corrects, self.correctNum = self.addPredOp(
                        ...
                        # compute gradients
                        gradient_vars = self.computeGradients(
                            self.optimizer, self.loss, trainableVars=None
                        )
        ...
        self.trainOp, self.gradNorm = self.addTrainingOp(
