...
1. The Control Unit computes the control state by computing attention over the
question words. The control state represents the current reasoning operation the
cell performs.

2. The Read Unit retrieves information from the knowledge base, given the control
and previous memory values, by computing 2-stages attention over the knowledge base.

3. The Write Unit integrates the retrieved information to the previous hidden memory
state, given the value of the control state, to perform the current reasoning operation.
"""
...
class MACCell(tf.nn.rnn_cell.RNNCell):
    ...
    Args:
        ...
        writeDropout: dropout on the new information that gets into the write unit
            (Tensor scalar).
    ...
    The unit is recurrent: it receives the whole question and the previous control
    state, merge them together (resulting in the "continuous control"), and then
    uses that to compute attentions over the question words. Finally, it combines
    the words together according to the attention distribution to get the new
    control state.
    ...
    def control(
        ...
        with tf.variable_scope("control" + name, reuse=reuse):
            ...
            # Step 1: compute "continuous" control state given previous control
            # and question.
            ...
            # Step 2: compute attention distribution over words and sum them up
            # accordingly.
            ...
            if config.controlProj:
                interactions = ops.linear(
                    interactions, dim, config.ctrlDim, act=config.controlProjAct
                )
                dim = config.ctrlDim
    ...
    Finally, it uses the attention distribution to sum up the knowledge base
    accordingly, resulting in an extraction of relevant information.
    ...
    def read(self, knowledgeBase, memory, control, name="", reuse=None):
        with tf.variable_scope("read" + name, reuse=reuse):
            ...
            if config.memoryVariationalDropout:
                memory = ops.applyVarDpMask(
                    memory, self.memDpMask, self.dropouts["memory"]
                )
            else:
            ...
            if config.readMemProj:
                interactions = ops.linear(
                    interactions, interDim, dim, act=config.readMemAct, name="memKbProj"
                )
            else:
                dim = interDim
            ...
            attention = ops.inter2att(
                interactions, dim, dropout=self.dropouts["read"], mask=self.kbSize
            )
    ...
    1. Self-attention to previous control / memory states, in order to consider
    previous steps in the reasoning process.
    2. Gating between the new memory and previous memory states, to allow dynamic
    adjustment of the reasoning process length.
    ...
    def write(self, memory, info, control, contControl=None, name="", reuse=None):
        with tf.variable_scope("write" + name, reuse=reuse):
            ...
            if config.writeSelfAtt:
                ...
                selfControl = ops.linear(
                    selfControl, config.ctrlDim, config.ctrlDim, name="ctrlProj"
                )

                interactions = self.controls * tf.expand_dims(selfControl, axis=1)

                attention = ops.inter2att(
                    interactions, config.ctrlDim, name="selfAttention"
                )
                self.attentions["self"].append(attention)
                selfSmry = ops.att2Smry(attention, self.memories)

            # get write unit inputs: previous memory, the new info, optionally
            ...
            if config.writeInputs == "INFO":
                ...
                newMemory, dim = ops.concat(
                    newMemory, info, dim, mul=config.writeConcatMul
                )
            # else: MEM
            ...
            if config.writeMergeCtrl:
                ...
                if config.writeMemProj or (dim != config.memDim):
                    newMemory = ops.linear(
                        newMemory, dim, config.memDim, name="newMemoryCtrl"
                    )
                    dim = config.memDim

                if config.writeMergeCtrlMul:
                    newMemory = tf.concat(
                        [newMemory, newMemory * control], axis=-1
                    )  # control,
                    dim += config.memDim  # 2 *
                else:
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope, reuse=self.reuse):  # as tfscope
            ...
            newControl, self.contControl = self.control(
                controlInput,
                inWords,
                outWords,
                self.questionLengths,
                control=control,
                contControl=self.contControl,
                name=cellName,
                reuse=cellReuse,
            )
            ...
            info = self.read(
                self.knowledgeBase, memory, newControl, name=cellName, reuse=cellReuse
            )
            ...
            self.controls = tf.concat(
                [self.controls, tf.expand_dims(newControl, axis=1)], axis=1
            )
            self.memories = tf.concat(
                [self.memories, tf.expand_dims(newMemory, axis=1)], axis=1
            )
    ...
    def initState(self, name, dim, initType, batchSize):
        if initType == "PRM":
            prm = tf.get_variable(
                name, shape=(dim,), initializer=tf.random_normal_initializer()
            )
    ...
    def zero_state(self, batchSize, dtype=tf.float32):
        ...
        # initialize state
        initialControl = self.initState(
            "initCtrl", config.ctrlDim, config.initCtrl, batchSize
        )
        initialMemory = self.initState(
            "initMem", config.memDim, config.initMem, batchSize
        )
        ...
        if config.initKBwithQ != "NON":
            if config.imageObjects:
                ...
                iVecQuestions = ops.linear(
                    self.vecQuestions, config.ctrlDim, config.memDim, name="questions"
                )
        ...
        if config.controlInWordsProj or config.controlOutWordsProj:
            pWords = ops.linear(
                words, config.wrdQEmbDim, config.ctrlDim, name="wordsProj"
            )
