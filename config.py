...
class Config(object):
    ...
    datasetFiles = lambda self, tier: [self.dataFile(fname) for fname in self.datasetFilenames(tier)]
...
def configCLEVR():
    ...
    Config.datasetFilenames = lambda self, tier: ["CLEVR_{tier}H_questions.json".format(tier=tier)]
...
def configGQA():
    ...
    Config.datasetFilenames = lambda self, tier: ["questions/{tier}_{dataSubset}_questions/{tier}_{dataSubset}_questions_{idx}.json".format(tier=tier, dataSubset=config.dataSubset, idx=i) for i in range(10)] if tier == "train" and config.dataSubset == "all" else ["questions/{tier}_{dataSubset}_questions.json".format(tier=tier, dataSubset=config.dataSubset)]
    config.wordVectorsFile = "{dataBasedir}/data/glove/glove.6B.{dim}d.txt".format(dataBasedir=config.dataBasedir, dim=config.wrdQEmbDim)
    config.wordVectorsSemanticFile = "{dataBasedir}/data/glove/glove.6B.{dim}d.txt".format(dataBasedir=config.dataBasedir, dim=config.semanticWordsEmbDim)
...
def configVQA():
    ...
    Config.datasetFilenames = lambda self, tier: ["{ver}_{ansFormat}_{tier}_questions.json".format(ver=dataVer, ansFormat=config.ansFormat, tier=tier)]
