...
def inter2att(
    interactions, dim, dropout=1.0, mask=None, sumMod="LIN", name="", reuse=None
):
    """
...
def expMask(seq, seqLength):
    ...
    mask = (tf.to_float(tf.logical_not(tf.sequence_mask(seqLength, maxLength)))) * (
        -inf
    )
    masked = seq + mask
    return masked
...
def seq2seqAcc(preds, targets, lengths):
    ...
    acc1 = tf.to_float(numCorrects) / (
        tf.to_float(lengths) + eps
    )  # add small eps instead?
...
# maybe change naming scheme such that if name = "" than use it as default_name
# (-->unique?)
def linear(
    inp,
    inDim,
    ...
    Args:
        ...
        transformationon top of previous
...
def CNNLayer(
    ...
    Args:
        ...
        batchNorm: if not None, applies batchNorm
        dropout: dropout value to apply for each layer
        kernelSizes: list of kernel sizes for each layer.
            Default to config.stemKernelSize
...
def locationL(h, w, dim, outDim=-1, addBias=True):
    ...
    grid = tf.stack(
        tf.meshgrid(
            tf.linspace(-config.locationBias, config.locationBias, w),
            tf.linspace(-config.locationBias, config.locationBias, h),
        ),
        axis=-1,
    )
...
def locationPE(h, w, dim, outDim=-1, addBias=True):
    ...
    """
    x = tf.expand_dims(
        tf.to_float(tf.linspace(-config.locationBias, config.locationBias, w)), axis=-1
    )
    y = tf.expand_dims(
        tf.to_float(tf.linspace(-config.locationBias, config.locationBias, h)), axis=-1
    )
...
locations = {"L": locationL, "PE": locationPE}
...
def linearizeFeatures(
    features, h, w, inDim, projDim=None, outDim=None, loc=None, pooling=None
):
    """
    ...
    [batchSize, outDim] (or [batchSize, (h / pooling) * (w /pooling) * projDim]
    if outDim not supported)
    """
    if pooling is None:
    ...
    if pooling > 1:
        ...
        features = tf.nn.max_pool(
            features, ksize=poolingDims, strides=poolingDims, padding="SAME"
        )
        h /= pooling
        w /= pooling
...
def mul(
    ...
    2. Computes simple multiplication, or a parametrized one, using diagonal of
        complete matrix (bi-linear)
...
def fwRNNLayer(
    inSeq, seqL, hDim, cellType=None, dropout=1.0, varDp=None, name="", reuse=None
):
    """
    ...
    with tf.variable_scope("rnnLayer" + name, reuse=reuse):
        ...
        #     if proj["output"]:
        #         outSeq = linear(
        #             outSeq, cell.output_size, proj["dim"], act = proj["act"],
        #             dropout = proj["dropout"], name = "projOutput"
        #         )
        #     if proj["state"]:
        #         lastState = linear(
        #             lastState, cell.state_size, proj["dim"], act = proj["act"],
...
def RNNLayer(
    ...
    return rnn(
        inSeq, seqL, hDim, cellType=cellType, dropout=dropout, varDp=varDp
    )  # , proj = proj
...
def gridRNNLayer(features, h, w, dim, right, down, name="", reuse=None):
    with tf.variable_scope("gridRNNLayer" + name):
        ...
        cell = createCell(
            dim, reuse=reuse, cellType=config.stemGridRnnMod, act=config.stemGridAct
        )
        ...
        for i in iAxis:
            for j in jAxis:
                prevs = tf.concat(
                    (prevState(i + iPrev, j), prevState(i, j + jPrev)), axis=-1
                )
                curr = inputs[i][j]
