...
class MiGRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def addBiases(self, inp1, inp2, dim, bInitial=0, name=""):
        with tf.variable_scope("additiveBiases" + name):
            b = (
                tf.get_variable(
                    "biases", shape=(dim,), initializer=tf.zeros_initializer()
                )
                + bInitial
            )
        with tf.variable_scope("multiplicativeBias" + name):
            beta = tf.get_variable(
                "biases", shape=(3 * dim,), initializer=tf.ones_initializer()
            )

        Wx, Uh, inter = tf.split(
            beta * tf.concat([inp1, inp2, inp1 * inp2], axis=1),
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope, reuse=self.reuse):
            ...
            r = tf.nn.sigmoid(
                self.addBiases(Wxr, Uhr, self.numUnits, bInitial=1, name="r")
            )
            ...
            u = tf.nn.sigmoid(
                self.addBiases(Wxu, Uhu, self.numUnits, bInitial=1, name="u")
            )
