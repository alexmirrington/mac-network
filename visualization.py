...
flatten = lambda ll: [e for x in ll for e in x]
...
inImgName = lambda instance: os.path.join(
    imagesDir, "{index}.jpg".format(index=instance["imageId"]["id"])
)

outImgAttName = lambda instance, j: os.path.join(
    outputDir, "{id}/Img_{step}.png".format(id=instance["index"], step=j + 1)
)

outTableAttName = lambda instance, name: os.path.join(
    outputDir,
