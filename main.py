...
from preprocess import Preprocesser, bcolored, bold, writeline, writelist
...
# region Printing, output and analysis
...
def fieldLenIsInRange(field):
    return lambda instance, group: (
        len(instance[field]) >= group[0] and len(instance[field]) <= group[1]
    )
...
def avg(instances, field):
    ...
    return sum(
        [
            (1 if instance["prediction"] == instance["answer"] else 0)
            for instance in instances
        ]
    ) / len(instances)
...
def printAnalysis(res):
    if config.analysisType != "":
        ...
        for key in groups:
            ...
            num = len(instances)
            print(
                "Group {key}: Loss: {loss}, Acc: {acc}, Num: {num}".format(
                    key=key, loss=0, acc=avgAcc, num=num
                )
            )
...
def printTierResults(tierName, res, color):
    ...
    print(
        "{tierName} Loss: {loss},".format(
            tierName=tierName, loss=bcolored(res["loss"], color)
        ),
        "{tierName} accuracy: {acc},".format(
            tierName=tierName, acc=bcolored(res["acc"], color)
        ),
        "minLoss: {minLoss},".format(minLoss=bcolored(res["minLoss"], color)),
        "maxAcc: {maxAcc}".format(maxAcc=bcolored(res["maxAcc"], color)),
    )
...
# endregion Printing, output and analysis
...
# Initializes savers (standard, optional exponential-moving-average and optional
# for subset of variables)
def setSavers(model):
    ...
    if config.saveSubset:
        ...
        subsetSaver = tf.train.Saver(
            relevantVars, max_to_keep=config.weightsToKeep, allow_empty=True
        )
    ...
    return {"saver": saver, "subsetSaver": subsetSaver, "emaSaver": emaSaver}


# endregion Savers


# region Restore / initialize weights
...
def loadWeights(sess, saver, init):
    if config.restoreEpoch > 0 or config.restore:
        ...
        print(
            bcolored(
                "Restoring epoch {} and lr {}".format(config.restoreEpoch, config.lr),
                "cyan",
            )
        )
...
# endregion Restore / initialize weights
...
def improveEnough(curr, prior, lr):
    ...
    if config.dataset == "CLEVR":
        notImprove = (
            (lossDiff < 0.015 and prevTrainLoss < 0.5 and lr > 0.00002)
            or (lossDiff < 0.008 and prevTrainLoss < 0.15 and lr > 0.00001)
            or (lossDiff < 0.003 and prevTrainLoss < 0.10 and lr > 0.000005)
        )
...
def loadImageBatch(images, batch):
    ...
    if config.imageObjects:
        imageBatch = np.zeros(
            (len(batch["imageIds"]), config.imageDims[0], config.imageDims[1])
        )
        for i, imageId in enumerate(batch["imageIds"]):
            ...
            imageBatch[i, 0:numObjects] = toFile(imageId)["features"][
                imageId["idx"], 0:numObjects
            ]

    else:
        imageBatch = np.stack(
            [
                toFile(imageId)["features"][imageId["idx"]]
                for imageId in batch["imageIds"]
            ],
            axis=0,
        )
...
def statsToStr(stats, res, epoch, batchNum, dataLen, startTime):
    formatStr = (
        "\reb {epoch},{batchNum} ({dataProcessed} / {dataLen:5d}), "
    ...
    return formatStr.format(
        epoch=s_epoch,
        batchNum=s_batchNum,
        dataProcessed=s_dataProcessed,
        dataLen=s_dataLen,
        time=s_time,
        loadTime=s_loadTime,
        trainTime=s_trainTime,
        lr=s_lr,
        loss=s_loss,
        acc=s_acc,
        avgLoss=s_avgLoss,
...
def runEpoch(
    ...
    if dataLen == 0:
        return {"loss": 0, "acc": 0, "maxAcc": 0, "minLoss": 0, "preds": []}
    ...
    while batchNum < batchesNum:
        try:
            ...
            # update stats
            stats = updateStats(stats, res)
            preds += res["preds"]

            sys.stdout.write(
                statsToStr(stats, res, epoch, batchNum, dataLen, startTime)
            )
            sys.stdout.flush()
...
def main():
    ...
    print(
        "took {} seconds".format(bcolored("{:.2f}".format(time.time() - start), "blue"))
    )
    ...
    with tf.Session(config=sessionConfig) as sess:
        ...
        if config.train:
            ...
            for epoch in range(config.restoreEpoch + 1, config.epochs + 1):
                ...
                prior = {
                    "best": {"res": bestRes, "epoch": bestEpoch},
            ...
            epoch -= 1
            print(
                "Training took {:.2f} seconds ({:} epochs)".format(
        ...
        if config.finalTest:
            ...
            evalRes = runEvaluation(
                sess, model, data["main"], dataOps, epoch, evalTest=False, getPreds=True
            )
            extraEvalRes = runEvaluation(
                sess,
                model,
        ...
        if config.interactive:
            ...
            while True:
                ...
                model.runBatch(
                    sess, datum, image, train=False, getPreds=True, getAtt=True
                )
